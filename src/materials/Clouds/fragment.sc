...
void main() {
  vec4 color = v_color0;
    ...
    #if NL_CLOUD_TYPE == 2
      vec3 origin = v_color0.xyz;

      color = renderCloudsRounded(vDir, origin, v_color1.w, v_color2.w, v_color2.rgb, v_color1.rgb, NL_CLOUD_PARAMS(_));

      #ifdef NL_CLOUD2_LAYER2
        float vy = max(abs(vDir.y), 0.01); // agar tidak terlalu kecil
        vec2 parallax = vDir.xz / vy * NL_CLOUD2_LAYER2_OFFSET;

        vec3 offsetPos = origin;
        ...
        color2.a = clamp(color2.a, 0.0, 1.0);

        color = mix(color2, color, 0.2 + 0.8 * color.a);
      #endif

      #ifdef NL_AURORA
        color += renderAurora(origin, v_color2.a, v_color1.a, v_fogColor) * (1.0 - 0.95 * color.a);
      #endif
